...
const TableAnswer: React.FC<TableAnswerProps> = ({
  ...
  const fetchAndSendSuccessMessage = async () => {
    try {
      await fetchData();
  ...
  useEffect(() => {
    if (matkul_id !== 0) {
      fetchData();
      const unsubscribe = subscribeToNewData();
      return unsubscribe;
    }
  }, [matkul_id]);
    ...
    <div className="mt-5 xl:mt-0 border rounded-md p-5 h-full flex flex-col">
      <div className="text-3xl font-bold text-white">
        ...
        <div className="font-bold text-lg mt-2">
          Data otomatis update ketika ada data baru.
        </div>
      </div>
                ...
                filteredData.map((item, index) => (
                  ...
                  >
                    <td className="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white border-r border-gray-300 dark:border-gray-600">
                      <div className="bg-blue-600 p-1 w-min h-min rounded-full text-xs">
                        {item.id}
                      </div>
                      <div className="mt-3">
